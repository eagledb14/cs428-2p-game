...
`,t)}paste(s){this._core.paste(s)}refresh(s,t){this._verifyIntegers(s,t),this._core.refresh(s,t)}reset(){this._core.reset()}clearTextureAtlas(){this._core.clearTextureAtlas()}loadAddon(s){this._addonManager.loadAddon(this,s)}static get strings(){return r}_verifyIntegers(...s){for(const t of s)if(t===1/0||isNaN(t)||t%1!=0)throw new Error("This API only accepts integers")}_verifyPositiveIntegers(...s){for(const t of s)if(t&&(t===1/0||isNaN(t)||t%1!=0||t<0))throw new Error("This API only accepts positive integers")}}P.Terminal=_})(),V})())})(Ae);var Ve=Ae.exports,Te={exports:{}};(function(ne,Q){(function(J,G){ne.exports=G()})(self,()=>(()=>{var J={};return(()=>{var G=J;Object.defineProperty(G,"__esModule",{value:!0}),G.FitAddon=void 0,G.FitAddon=class{activate(j){this._terminal=j}dispose(){}fit(){const j=this.proposeDimensions();if(!j||!this._terminal||isNaN(j.cols)||isNaN(j.rows))return;const V=this._terminal._core;this._terminal.rows===j.rows&&this._terminal.cols===j.cols||(V._renderService.clear(),this._terminal.resize(j.cols,j.rows))}proposeDimensions(){if(!this._terminal||!this._terminal.element||!this._terminal.element.parentElement)return;const j=this._terminal._core,V=j._renderService.dimensions;if(V.css.cell.width===0||V.css.cell.height===0)return;const P=this._terminal.options.scrollback===0?0:j.viewport.scrollBarWidth,r=window.getComputedStyle(this._terminal.element.parentElement),o=parseInt(r.getPropertyValue("height")),c=Math.max(0,parseInt(r.getPropertyValue("width"))),u=window.getComputedStyle(this._terminal.element),n=o-(parseInt(u.getPropertyValue("padding-top"))+parseInt(u.getPropertyValue("padding-bottom"))),d=c-(parseInt(u.getPropertyValue("padding-right"))+parseInt(u.getPropertyValue("padding-left")))-P;return{cols:Math.max(2,Math.floor(d/V.css.cell.width)),rows:Math.max(1,Math.floor(n/V.css.cell.height))}}}})(),J})())})(Te);var Ge=Te.exports;const Xe={border:"t base",flex:"~ gap-2","items-center":"",p2:""},Je={"text-sm":"",op50:""},Ye=we({__name:"TerminalView",props:{id:{}},setup(ne){const Q=ne,J=Ee(),G=He(),j=Ee();let V;Oe(async()=>{V=new Ve.Terminal({convertEol:!0,cols:80,screenReaderMode:!0});const r=new Ge.FitAddon;V.loadAddon(r),V.open(J.value),r.fit(),Pe(window,"resize",()=>{r.fit()}),j.value=await fe.getTerminalDetail(await Ie(),Q.id),j.value?.buffer&&V.write(j.value.buffer),G.hook("devtools:terminal:data",({id:o,data:c})=>{o===Q.id&&V.write(c)})});function P(){fe.runTerminalAction(Q.id,"clear"),V?.clear()}return(r,o)=>{const c=xe;return te(),he(Re,null,[oe("div",{ref_key:"container",ref:J,"h-full":"","w-full":"","of-auto":"","bg-black":""},null,512),oe("div",Xe,[De(c,{title:"Clear",icon:"i-carbon-clean",onClick:o[0]||(o[0]=u=>P())}),Z(j)?.restartable?(te(),ce(c,{key:0,title:"Restart",icon:"carbon-renew",onClick:o[1]||(o[1]=u=>("rpc"in r?r.rpc:Z(fe)).runTerminalAction(r.id,"restart"))})):ge("",!0),Z(j)?.terminatable?(te(),ce(c,{key:1,title:"Terminate",icon:"carbon-delete",onClick:o[2]||(o[2]=u=>("rpc"in r?r.rpc:Z(fe)).runTerminalAction(r.id,"terminate"))})):ge("",!0),oe("span",Je,pe(Z(j)?.description),1)])],64)}}}),Ze={key:0,"h-full":"","w-full":"","of-hidden":"",grid:"~ rows-[max-content_1fr_max-content]"},Qe={flex:"~",border:"b base","flex-1":"","items-center":"","navbar-glass":""},et=["onClick"],tt={key:1,p10:""},it={key:1,"h-full":"",flex:"","items-center":"","justify-center":""},st=oe("em",{op50:""},"No terminal attached",-1),rt=[st],nt=we({__name:"TerminalPage",setup(ne){const Q=Fe(),J=We(),G=Ue(()=>Q.value?.find(V=>V.id===J.value));function j(V){fe.runTerminalAction(V,"remove")}return Ne(()=>{!J.value&&Q.value?.length&&(J.value=Q.value[0].id)}),(V,P)=>{const r=ze,o=xe,c=Ye;return Z(Q)?.length?(te(),he("div",Ze,[oe("div",Qe,[(te(!0),he(Re,null,$e(Z(Q),u=>(te(),he("button",{key:u.id,border:"r base",flex:"~ gap-2","items-center":"",px3:"",py2:"",class:ke(u.id===Z(G)?.id?"bg-active":""),onClick:n=>J.value=u.id},[u.icon?(te(),ce(r,{key:0,icon:u.icon},null,8,["icon"])):ge("",!0),oe("span",{class:ke(u.id===Z(G)?.id?"":"op50")},pe(u.name)+pe(u.isTerminated?" (terminated)":""),3),u.isTerminated?(te(),ce(o,{key:1,icon:"carbon-close","mx--2":"",onClick:je(n=>j(u.id),["stop"])},null,8,["onClick"])):ge("",!0)],10,et))),128))]),Z(G)?(te(),ce(c,{id:Z(G).id,key:Z(G).id},null,8,["id"])):(te(),he("div",tt,[Le(" Terminal "),oe("code",null,pe(Z(J)),1),Le(" not found ")]))])):(te(),he("div",it,rt))}}});const at=we({__name:"terminals",setup(ne){return(Q,J)=>{const G=nt,j=qe;return te(),ce(j,null,{default:Ke(()=>[De(G)]),_:1})}}});export{at as default};
